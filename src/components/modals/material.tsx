...
export function AddMaterialModal({
  ...
  const upload = () => {
    ...
    newAxiosInstance
      .post(`${config.REST_API_URL}/material/upload`, formData)
      .then((res) => {})
...
export function EditMaterialModal({
  ...
  useEffect(() => {
    // Fetch Data to Get Material Detail
    const fetchData = async () => {
      try {
        setIsLoading(true);
        const res = await newAxiosInstance.get(
          `${config.REST_API_URL}/material/${materialId}`
        );
  ...
  const upload = () => {
    ...
    if (oldFile) {
      ...
      newAxiosInstance
        .delete(`${config.REST_API_URL}/material/deleteFile/${oldFile}`)
        .then(() => {
